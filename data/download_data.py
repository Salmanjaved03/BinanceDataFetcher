...
class DataDownloader:
    ...
    def resample_data(self, rule='H'):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name=?;", (fetcher.table_name,))
        table_exists = cursor.fetchone()
        conn.close()

        # If table does not exist, fetch data
        if not table_exists:
            print(f"[INFO] Table '{fetcher.table_name}' not found in DB. Fetching data from '{fetcher.exchange}' API...")
            fetcher.fetch_data()
    ...
    def fetch_resampled(self, rule='H'):
        resampled_df = self.resample_data(rule)
        start = pd.to_datetime(self.start_date, utc=True)
        end = pd.to_datetime(datetime.utcnow() if self.end_date == "now" else self.end_date, utc=True)
        filtered_df = resampled_df[(resampled_df['datetime'] >= start) & (resampled_df['datetime'] <=end)]
